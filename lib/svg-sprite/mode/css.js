...
SVGSpriteCss.prototype._layoutSimple = function(pseudoShapeMap) {
	var padding = this._spriter.config.shape.spacing.padding.right || 0;
	this._spriter._shapes.forEach(function(shape, index){
		if (this._displaceable || !shape.master) {
			this._addShapeToSimpleCssSprite(shape, pseudoShapeMap[shape.base], index, !index + (index === this._spriter._shapes.length - 1) * 2, padding);
		}
	}, this);
	return this;
};
 ...
 */
SVGSpriteCss.prototype._addShapeToSimpleCssSprite = function(shape, needsRegular, index, position, padding) {
	...
	switch (this.config.layout) {
		...
		case this.LAYOUT_HORIZONTAL:
			...
			this.data.spriteWidth	= Math.ceil(this.data.spriteWidth + dimensions.width + padding);
		...
		case this.LAYOUT_DIAGONAL:
			...
			this.data.spriteWidth	= Math.ceil(this.data.spriteWidth + dimensions.width + padding);
			this.data.spriteHeight	= Math.ceil(this.data.spriteHeight + dimensions.height + padding);
			break;
...
SVGSpriteCss.prototype._layoutBinPacked = function(pseudoShapeMap) {
	...
	packer							= new SVGSpriteCssPacker(this._spriter._shapes, padding),
